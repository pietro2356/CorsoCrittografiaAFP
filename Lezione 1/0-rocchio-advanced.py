...
| NOTE: - This version handles: Lowercase Characters, Upper Case, Numbers, Special Characters and generate random keys with each new encryption.
\------------------------------------------
'''
...
def write_file(filename, txt):
    try:
        # Open in read text mode.
        with open('Lezione 1\\' + filename, "w") as out_file:
            out_file.write(txt)
    except IOError as e:
        raise SubCipherError('Error: cannot write ' + filename +' file: ' + str(e))
    
    print('Encrypted message correctly and saved in -> ' + filename +'\n')
...
''' ---- Random Key Generate---- '''
def rndKeyGenerator(wordLen):
    key = ""
    for item in range(0, wordLen):
        key += alphList[random.randint(0, len(alphList) - 1)]
...
def encrypt():    
    ...
    printResultEncription(word, key, cipherText)
    return cipherText
